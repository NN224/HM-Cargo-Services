...
const SCENES: SceneCopy[] = [
  { eyebrow: "Cargo without uncertainty", title: "Moving your cargo forward.", body: "Reliable logistics, clear visibility, and careful handling from origin to destination." },
  { eyebrow: "Origin", title: "Ready at the source.", body: "Organized handling from the very first step of the journey." },
  {
    eyebrow: "Warehouse",
    title: "Controlled at every handoff.",
    body: "Careful warehouse processing and shipment coordination across every stage.",
  },
  { eyebrow: "Transport", title: "Built to keep moving.", body: "Reliable ground transportation through every leg of the route." },
  { eyebrow: "Port", title: "Connected beyond borders.", body: "Cargo solutions designed for regional and international movement." },
  { eyebrow: "Visibility", title: "Visibility from origin to arrival.", body: "Clear updates and confidence throughout the shipment lifecycle." },
  { eyebrow: "Arrival", title: "Delivered with intent.", body: "Every shipment ends where it began — with care." },
];
...
export function CinematicJourney() {
      ...
      ctx = gsap.context(() => {
        ...
        const pinTl = gsap.timeline({
          scrollTrigger: {
            ...
            anticipatePin: 1,
            onUpdate: (self) => {
              const active = Math.min(sceneCount - 1, Math.floor(self.progress * sceneCount));
              dots.forEach((d, i) => {
        ...
        for (let i = 0; i < sceneCount; i++) {
          ...
          // Copy in
          pinTl.to(copyEls[i], { opacity: 1, y: 0, duration: step * 0.25, ease: "power2.out" }, start);
          // Copy hold, then out — never overlap into next scene
          pinTl.to(copyEls[i], { opacity: 0, y: -20, duration: step * 0.2, ease: "power2.in" }, outStart);

          // Scene layer in (skip 0 already visible)
          if (i > 0) {
            pinTl.to(layers[i], { opacity: 1, scale: 1, duration: step * 0.35, ease: "power2.inOut" }, start - step * 0.05);
          }
          // Scene layer subtle forward push during its window
          pinTl.to(layers[i], { scale: 1.08, duration: step, ease: "none" }, start);
          // Previous scene out
          if (i > 0) {
            pinTl.to(layers[i - 1], { opacity: 0, duration: step * 0.35, ease: "power2.inOut" }, start - step * 0.02);
          }
          ...
          if (i === 0) {
            ...
            if (line) {
              ...
              pinTl.to(line, { strokeDashoffset: 0, duration: step * 0.6, ease: "power2.out" }, mid - step * 0.3);
            }
          }
          ...
          if (i === 5) {
            ...
            if (route) {
              ...
              pinTl.to(route, { strokeDashoffset: 0, duration: step * 0.9, ease: "power2.inOut" }, start);
            }
          }
        }
      }, rootRef);
  ...
  return (
    ...
    >
      {/* Scene layers */}
      <SceneOpening />
      <SceneImage src={sceneContainer} alt="Sealed cargo container in a dark industrial space" />
      <SceneImage src={sceneWarehouse} alt="Dark industrial warehouse with orange guide lights" />
        ...
        <div className="mx-auto w-full max-w-[1440px] px-6 lg:px-16">
          {SCENES.map((s, i) => (
            ...
            >
              ...
              <h2 className={`mt-5 font-display ${i === 0 ? "text-5xl sm:text-6xl lg:text-8xl" : "text-4xl sm:text-5xl lg:text-7xl"} text-foreground`}>
                {s.title}
              </h2>
      ...
      <div className="pointer-events-none absolute left-8 top-1/2 z-30 hidden -translate-y-1/2 lg:block">
        <div className="relative h-72 w-px bg-[color:var(--border)]">
          <div ref={progressRef} className="absolute left-0 top-0 w-px bg-[color:var(--accent)]" style={{ height: "0%" }} />
          {SCENES.map((_, i) => (
            <span
              key={i}
              data-dot
      ...
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 1600 900" preserveAspectRatio="none" aria-hidden>
        ...
        {[
          ...
          <g key={i}>
            ...
            <circle cx={x} cy={y} r="10" fill="none" stroke="#FF7900" strokeOpacity="0.35" />
          </g>
        ))}
      </svg>
    </div>

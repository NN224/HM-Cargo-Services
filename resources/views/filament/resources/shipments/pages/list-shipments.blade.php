                ...
                @if (count($this->selectedShipmentIds) > 0)
                    <div class="hm-bulk-bar">
                        ...
                        <div class="hm-bulk-actions">
                            ...
                            @if (filled($this->bulkTargetStatus) && auth()->user()?->isAdministrator())
                                <label class="hm-bulk-field hm-bulk-reason">
                                    <span>سبب التصحيح (اختياري)</span>
                                    <input type="text" wire:model.live="bulkCorrectionReason" placeholder="يمكن تركه فارغاً">
                                </label>
                            @endif
                            <button
                                type="button"
                                class="hm-primary-btn"
                                wire:click="bulkAdvanceSelectedShipments"
                                wire:loading.attr="disabled"
                                wire:target="bulkAdvanceSelectedShipments"
                            >تحديث {{ count($this->selectedShipmentIds) }} شحنات</button>
                            <button type="button" class="hm-ghost-btn" wire:click="clearShipmentSelection">إلغاء</button>
                        </div>
                    </div>
                @endif
                        ...
                        @foreach ($visibleShipments as $shipment)
                            @php
                                ...
                                foreach ($steps as $idx => $step) {
                                    if ($step['completed_count'] > 0) $lastDone = $idx;
                            ...
                            <article class="hm-shipment-card {{ $isSelected ? 'is-selected' : '' }}">
                                ...
                                <div class="hm-detail-grid">
                                    ...
                                    <div><span>الحالة</span><strong class="hm-neutral-badge">{{ $shipment->status->label() }}</strong></div>
                                ...
                                <div class="hm-card-footer">
                                    ...
                                    <div class="hm-card-actions">
                                        ...
                                        <a class="hm-icon-btn" href="{{ ShipmentResource::getUrl('view', ['record' => $shipment]) }}">•••</a>
                                    </div>
                                </div>
                            </article>
                        @endforeach
